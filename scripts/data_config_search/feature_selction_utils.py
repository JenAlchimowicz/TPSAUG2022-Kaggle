...
class FeatureSelector():
    ...
    def transform(self, X):
        ...
        # Exceptions
        if 'measurement_3_missing' in self.features_to_delete:
            self.features_to_delete.remove('measurement_3_missing')
        if 'measurement_5_missing' in self.features_to_delete:
            self.features_to_delete.remove('measurement_5_missing')

        else:
            # Get selected features
            X = X.drop(columns=self.features_to_delete)
